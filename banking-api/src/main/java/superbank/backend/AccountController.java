...
class AccountController {
	...
	ResponseEntity<ObjectNode> accountStatus(@PathVariable(name = "accountId") String accountId, @RequestHeader(name = "Authorization") String authHeader) {
		String token = AuthHeader.getBearerToken(authHeader);

		//TODO add service data to input

		ObjectNode input = new ObjectMapper().createObjectNode();
		input.put("jwt", token);
		input.put("action", "account/status");
		input.put("accountId", accountId);
		Boolean allowed = opaClient.queryForDocument(new QueryForDocumentRequest(input, "entitlements/allowed"), Boolean.class);
	...
	ResponseEntity<ObjectNode> accountTransactions(@PathVariable(name = "accountId") String accountId, @RequestHeader(name = "Authorization") String authHeader) {
		String token = AuthHeader.getBearerToken(authHeader);

		ObjectNode input = new ObjectMapper().createObjectNode();
		input.put("jwt", token);
		input.put("action", "account/transactions");
		input.put("accountId", accountId);
		Boolean allowed = opaClient.queryForDocument(new QueryForDocumentRequest(input, "entitlements/allowed"), Boolean.class);
